...
- アプリは**単一バイナリで自己完結（Docker 不要）**。LLM は **vLLM**（GPU）。既定モデル `Qwen3-4B`(~8GB) は **RTX 4000 Ada(20GB) に余裕で載る**。
- **bare-metal native** 構成: PostgreSQL もホストに入れ、**data_directory を Volume に移設**して永続化（§2-3）。一式を **Image に焼けば restore は `db start` だけ**。

### アーキテクチャ

```
┌──────────────── GPU インスタンス（使い捨て / Image 化）──────────┐
│  OS / NVIDIA driver+CUDA / PostgreSQL / vLLM venv / app バイナリ │
│                                                                 │
│   DigitalBase app ──▶ vLLM (chat:8080 / embed:8081)  ← GPU      │
│         │                                                       │
│         └──▶ PostgreSQL(pgvector)  ← host native（apt）          │
└──────────────────────────────│──────────────────────────────────┘
              data_directory 等  │  はすべて Volume を指す
        ┌─────────────────────▼──────────────────┐
        │  Block Storage Volume  /mnt/db （永続） │
        │   ├─ pgdata/          ← SQL（PG data_directory）
        │   ├─ files/           ← アップロード     │
        │   ├─ hf-cache/        ← モデル          │
        │   ├─ .env                              │
        │   └─ license.lic                       │
...
| Container runtime | bare-metal native では**不要** | Docker-PG 代替（§4 末尾）を使う場合のみ Docker + NVIDIA Container Toolkit |

### 使用イメージ（Docker Hub）

> 既定の **bare-metal native** では **PostgreSQL=apt の `postgresql-16` / アプリ=単一バイナリ / vLLM=native venv** を使うため、下表のイメージは**基本不要**。Docker ベースの代替（Docker-PG 等）を使う場合の参照。
...
GPU を「使うときだけ」にすれば、保持コストは月 $5 前後に収まる（restore 高速化用に Image を保存する場合は **+$0.5〜0.8/月**）。
...
- **PostgreSQL のメジャーバージョン固定**: ホストの **PG 16（`postgresql-16`）のまま**。`/mnt/db/pgdata` を再利用するので PG17 等に上げると起動しない（上げる時は `pg_upgrade`）。
- **ポータビリティは維持**: 本構成は 単一バイナリ + PostgreSQL + vLLM で完結しており Akamai 固有ではない。Akamai をコスト最適な既定としつつ、AWS / オンプレも選べる状態を保つ（ロックイン回避＋交渉力）。

---

## 代替構成（必要に応じて）

- **PostgreSQL を Akamai Managed Database（Aiven）に**: SQL が常時マネージドで永続（インスタンス削除と無関係）。Block Storage はファイル＋モデルだけでよくなる。最小 Shared 1GB で **+$16/月**（HA 3ノードで +$37）。data_directory 移設も不要になり運用は最も楽だが、アイドル時も課金され続ける。
- **Docker-PG**: §4 末尾参照。Volume 永続化が `-v` 1行で済むが restore に `docker run` が要る。
