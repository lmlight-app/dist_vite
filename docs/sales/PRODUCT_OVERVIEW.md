...
- 主要オペレータ:
  - **ファイル入出力**: File Hub の全接続先（S3 / GCS / Azure Blob / SFTP / FTP・FTPS / SMB / WebDAV / HTTP / Google Drive / OneDrive / Dropbox / Box / SharePoint）を「ファイル取得」「ファイル保存」から直接参照（接続は File Hub と共通・再利用）、Google Sheets, rsync
  ...
  - **AI処理**: LLM, AI分類, 画像を AI で読み取り（Vision）, RAGロード / 検索, 文書比較
  - **データ操作**: Filter, Sort, Aggregate, Merge, Split, Limit, Dedup, Compare, Validate, Cast, Rename Keys
...
インストール先: `~/.local/db`（Ollama / vLLM 共通。単一バイナリで `.env` の `LLM_BACKEND` により切替）
