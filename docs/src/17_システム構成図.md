...
graph TB
    subgraph "お客様の環境"
        ...
        subgraph "オプション機能"
            ...
            Vision["Vision / OCR<br/>Vision LLM + Tesseract"]
        end
...
| 文字起こし | faster-whisper（CTranslate2、CUDA / CPU int8、VAD 付き）/ pywhispercpp（whisper.cpp、CPU / Metal）。モデル形式で自動選択 |
| Vision LLM | Ollama / vLLM 上で動作する Vision 対応モデル（Qwen2.5-VL / Gemma 3 / DeepSeek-OCR / Granite Vision など） |
| OCR | Tesseract（jpn+eng） |
| BI / Dashboard | react-grid-layout (Canvas) + recharts |
